...
class Game:
    ...
    def __init__(self, seconds_left=300):
        self.turn = chess.WHITE  # True for white, False for black

        self.board_is_real = chess.Board()
        ...
        self.time_left_color = {chess.WHITE: seconds_left, chess.BLACK: seconds_left}
        self.curr_time = None
    ...
    def end(self):
        ...
        Ends the game.
        """
        self.time_left_color[self.turn] = self.get_seconds_left()
    ...
    def get_seconds_left(self):
        ...
        """
        if not self.is_finished and self.curr_time:
            elapsed_since_turn_start = (datetime.now() - self.curr_time).total_seconds()
            return self.time_left_color[self.turn] - elapsed_since_turn_start
        else:
            return self.time_left_color[self.turn]

    ###=== Generate Legal Moves ===###
    def _no_opp_pieces(self, board, turn):
        """
    ...
    def _moves_no_opp_pieces(self, board, turn):
        """
        ...
        """
        return list(self._no_opp_pieces(board, turn).generate_pseudo_legal_moves())
    ...
    def get_moves(self):
        ...
        return self._moves_no_opp_pieces(self.board_is_real, self.turn) + \
               self._pawn_on(self.board_is_real, self.turn)

    ###=== Make move and update board ===###
    def _where_are_captured_pieces(self, board, move):
        """
    ...
    def _add_pawn_queen_promotion(self, move):
        ...
        piece = self.board_is_real.piece_at(move.from_square)
    ...
    def _fixing_move(self, move):
        ...
        if move in self.board_is_real.generate_pseudo_legal_moves() or self._is_psuedo_legal_castle(self.board_is_real, move):
            return move

        # note: if there are pieces in the way, we DONT capture them
        if self._is_illegal_castle(self.board_is_real, move):
            return None
        ...
        if piece.piece_type in [chess.PAWN, chess.ROOK, chess.BISHOP, chess.QUEEN]:
            move = self._slide_move(self.board_is_real, move)

        return move if move in self.board_is_real.generate_pseudo_legal_moves() else None
    ...
    def handle_move(self, requested_move):
        ...
        if requested_move is None:
            ...
            taken_move = self._fixing_move(move)
            captured_square = self._where_are_captured_pieces(self.board_is_real, taken_move)
            reason = ""

        # push move to appropriate boards for updates #
        self.board_is_real.push(taken_move if taken_move is not None else chess.Move.null())
        ...
        if self.turn == chess.WHITE:
            self.white_board.set_fen(self._no_opp_pieces(self.board_is_real, self.turn).fen())
            self.black_board.set_fen(self._no_opp_pieces(self.board_is_real, not self.turn).fen())
        else:
            self.black_board.set_fen(self._no_opp_pieces(self.board_is_real, self.turn).fen())
            self.white_board.set_fen(self._no_opp_pieces(self.board_is_real, not self.turn).fen())
        ...
        for delta_rank in [1, 0, -1]:
            for delta_file in [-1, 0, 1]:
                if 0 <= rank + delta_rank <= 7 and 0 <= file + delta_file <= 7:
                    ...
                    sense_result.append((sense_square, self.board_is_real.piece_at(sense_square)))
    ...
    def end_turn(self):
        ...
        elapsed = datetime.now() - self.curr_time
        self.time_left_color[self.turn] -= elapsed.total_seconds()
    ...
    def is_over(self):
        ...
        no_time_left = self.time_left_color[chess.WHITE] <= 0 or self.time_left_color[chess.BLACK] <= 0
        king_captured = self.board_is_real.king(chess.WHITE) is None or self.board_is_real.king(chess.BLACK) is None
    ...
    def get_winner(self):
        ...
        if self.time_left_color[chess.WHITE] <= 0:
            return chess.BLACK, "BLACK won by timeout"
        elif self.time_left_color[chess.BLACK] <= 0:
            return chess.WHITE, "WHITE won by timeout"

        if self.board_is_real.king(chess.WHITE) is None:
            return chess.BLACK, "BLACK won by king capture."
        elif self.board_is_real.king(chess.BLACK) is None:
